...
$scriptBuffer = @()  # Zwischenspeicher für den Skriptstream
...
function Execute-Script {
    param ([string]$scriptName, [array]$scriptLines)

    $tempFile = "$env:TEMP\$scriptName"
    $scriptLines -join "`r`n" | Out-File -Encoding utf8 $tempFile

    Write-Host "▶️ Skript wird ausgeführt: $tempFile" -ForegroundColor Cyan
...
function Listen-WebSocket {
    ...
    while ($true) {
        ...
        try {
            ...
            try {
                $data = $message | ConvertFrom-Json -ErrorAction SilentlyContinue
            } catch {
                $data = $null
            }

            if ($data -and $data.action -eq "execute_script") {
                if ($data.content -eq "[EOF]") {
                    # Skript wurde vollständig übertragen – jetzt ausführen
                    Write-Host "✅ Skriptübertragung abgeschlossen. Starte Ausführung..." -ForegroundColor Green
                    Execute-Script -scriptName $data.script_name -scriptLines $scriptBuffer
                    $scriptBuffer = @()  # Buffer zurücksetzen
                } else {
                    # Skriptinhalt in den Buffer schreiben
                    Write-Host "📜 Zeile empfangen: $($data.content)" -ForegroundColor Magenta
                    $scriptBuffer += $data.content
                }
            } elseif ($message -match "registered") {
...
# Starte den WebSocket-Client mit Wiederverbindungsversuch
$websocket = Connect-WebSocket -serverUrl $serverUrl
