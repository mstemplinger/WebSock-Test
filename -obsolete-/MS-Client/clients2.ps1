    ...
    while ($true) {
        ...
        try {
            ...
            if ($data -and $data.action -eq "execute_script") {
                Write-Host "🛠 Skript zum Ausführen erhalten: $($data.script_name)" -ForegroundColor Magenta
                Execute-Script -scriptName $data.script_name -scriptContent $data.script_content
